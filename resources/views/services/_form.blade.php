<div class="grid grid-cols-1 md:grid-cols-2 gap-6">
    ...
    <div class="col-span-1" id="start-date-group" style="display: none;">
        ...
        <input type="date" name="start_date" id="start_date" 
               value="{{ old('start_date', isset($service->start_date) ? $service->start_date->format('Y-m-d') : '') }}"
               onchange="const endDateInput = document.getElementById('end_date'); if(!endDateInput.value || endDateInput.value < this.value) endDateInput.value = this.value;"
               class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
        @error('start_date')
            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
        @enderror
