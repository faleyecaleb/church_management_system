    ...
    <div class="max-w-7xl mx-auto py-6">
        ...
        <div class="bg-white shadow overflow-hidden sm:rounded-lg mb-6">
            <div class="px-4 py-5 sm:px-6 flex justify-between items-start">
                ...
                <img class="h-16 w-16 rounded-full ring-4 ring-white sm:h-24 sm:w-24" src="{{ $member->profile_photo_url }}" alt="{{ $member->full_name }}">
                <div class="ml-4">
                    <h1 class="text-2xl font-bold text-gray-900 truncate">{{ $member->full_name }}</h1>
        ...
        <div class="bg-white shadow overflow-hidden sm:rounded-lg mb-6">
            <div class="px-4 py-5 sm:px-6 flex justify-between items-center">
                ...
                @can('member.update')
                    <button type="button"
                        onclick="document.getElementById('emergency-contacts-modal').classList.remove('hidden')"
                        class="inline-flex items-center px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                        <svg class="-ml-1 mr-2 h-5 w-5 text-gray-500" xmlns="http://www.w3.org/2000/svg" fill="none"
                            viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    ...
                                    <div class="flex space-x-4">
                                        ...
                                        @if($contact->email)
                                            <a href="mailto:{{ $contact->email }}"
                                                class="text-indigo-600 hover:text-indigo-900">{{ $contact->email }}</a>
                                        @endif
                                    </div>
                                </div>
                            </div>
        ...
        <div class="bg-white shadow overflow-hidden sm:rounded-lg">
            <div class="px-4 py-5 sm:px-6 flex justify-between items-center">
                ...
                @can('member.update')
                    <button type="button" onclick="document.getElementById('document-upload-modal').classList.remove('hidden')"
                        class="inline-flex items-center px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                        <svg class="-ml-1 mr-2 h-5 w-5 text-gray-500" xmlns="http://www.w3.org/2000/svg" fill="none"
                            viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        ...
                        <li class="px-4 py-4 sm:px-6">
                            <div class="flex items-center justify-between">
                                ...
                                <div class="ml-4 flex-shrink-0 flex space-x-4">
                                    ...
                                    @can('member.update')
                                        <form action="{{ route('members.documents.delete', [$member, $document]) }}" method="POST"
                                            class="inline">
                                            @csrf
                                            @method('DELETE')
